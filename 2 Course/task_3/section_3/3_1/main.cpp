...
template <typename T, class _Cmp>
void AVLTree<T, _Cmp>::del_i(uint_fast32_t num) {
...
int main() {
    ...
    AVLTree<uint_fast32_t, std::greater<>> soldiers{};
    ...
    for (auto i = 0; i < n; ++i) {
        ...
        if (command == 1) {
            results.push_back(soldiers.insert(value));
        }
        else
            soldiers.del_i(value);
    }
